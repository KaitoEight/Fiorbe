...
import UITer from "./models/User.js";
...
app.use(cors({
    credentials: true,
    origin: ["http://localhost:5173","http://localhost:5174", "https://jsm-contest-fr-1.vercel.app", "https://675248c9d1983b9597b5c6ac.mockapi.io"],
}));
mongoose.connect("mongodb+srv://23560056:minhkhanh1@cluster0.hubbt.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0");
...
app.post('/register', async (req, res) => {
    ...
    try {
        const userDoc = await UITer.create({
...
app.post('/login', async (req, res) => {
    ...
    const userDoc = await UITer.findOne({email})
...
const findUserByEmail = async (email) => {
    return await UITer.findOne({ email:email });
  };
const findUserByEmailAndUpdate = async (email, data) => {
    return await UITer.findOneAndUpdate({ email }, {data}, {new: true});
  };
...
app.put("/updateUser/:email", async(req,res)=>{
    ...
    const foodupdate = await UITer.findOneAndUpdate({ email }, req.body, { new: true });
...
app.post("/cart/add", async (req, res) => {
    ...
    try {
        // Find user by email
        const user = await UITer.findOne({ email });
...
app.get("/cart/:email", async (req, res) => {
    ...
    try {
        // Find user by email
        const user = await UITer.findOne({ email }).populate('cartItems.productId'); // Optional: populate product details
...
app.post("/order/create", async (req, res) => {
    ...
    try {
      ...
      const user = await UITer.findOne({ email }).populate('cartItems.productId');
